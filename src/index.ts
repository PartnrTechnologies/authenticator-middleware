...
export enum Scope {
  ...
  FUNDS_GET = "@funds/get",
  FUNDS_REPORTS_GET = "@funds/reports/get",
  FUNDS_UNITHOLDERS_GET = "@funds/unitholders/get",
  FUNDS_UNITHOLDERS_CONCENTRATION_GET = "@funds/unitholders/concentration/get",
  FUNDS_RATIOS_GET = "@funds/ratios/get",
  FUNDS_RATIOS_VALUATION_GET = "@funds/ratios/valuation/get",
  FUNDS_PORTFOLIO_GET = "@funds/portfolio/get",
  FUNDS_TRADES_GET = "@funds/trades/get",
  FUNDS_UNITS_HISTORY_GET = "@funds/units-history/get",
  FUNDS_FUND_FLOW_GET = "@funds/fund-flow/get",
  FUNDS_NAV_GET = "@funds/nav/get",
  FUNDS_GOVERNANCE_GET = "@funds/governance/get",
  FUNDS_CREDIT_QUALITY_GET = "@funds/credit-quality/get",
  FUNDS_CASH_CORPORATE_ACTIONS_GET = "@funds/cash-corporate-actions/get",
  FUNDS_CORPORATE_ACTIONS_GET = "@funds/corporate-actions/get",
...
function getNormalizedRequestParam(req, paramKey: string): string[] {
  return [
...
function hasConstrainedQueryScope(scope: Scope, userScopes: string[], req): boolean {
  ...
  for (const [paramKey, allowedValues] of allowedValuesByParam.entries()) {
    ...
    if (req.query) req.query[paramKey] = fixedValue
    if (req.body && typeof req.body === 'object') req.body[paramKey] = fixedValue
  }
...
function resolveQuoteType(req): 'current' | 'historical' | 'chart' {
  const raw = req.body?.type ?? req.query?.type
  const value = typeof raw === 'string' ? raw.trim().toLowerCase() : ''
  if (value === 'current' || value === 'chart') return value
  return 'historical'
}

function ensureQuotesScope(umbrella: Scope, eodScope: Scope, delayScope: Scope) {
  return (req, res, next) => {
    const userScopes = res.locals.scopes
